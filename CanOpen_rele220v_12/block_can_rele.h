...
switch (tab->type){
	...
	if(tab->object == UNSIGNED8)d += *((uint8_t *)(tab->data));  
	if(tab->object == UNSIGNED16) d += *((uint16_t *)(tab->data));
	if(tab->object == UNSIGNED32) d = *((uint32_t *)(tab->data));
	break;

	case OD_ARRAY:
		
	arr = (struct OD_array_n *)(tab->data);
	sub_index = arr->sub_index ;
	if(sub_index != 0 ){sub_index--;}else{break;}
	if(tab->object == UNSIGNED8){d +=  *((uint8_t*)(arr->value)+(sub_index));break;} 
	if(tab->object == UNSIGNED16){d += *((uint16_t *)(arr->value)+(sub_index));break;}
	if(tab->object == UNSIGNED32) d =*((uint32_t *)(arr->value)+(sub_index));
	break;
	
	case OD_DEFSTRUCT:
	
	if(tab->object == PDO_COMM){};
