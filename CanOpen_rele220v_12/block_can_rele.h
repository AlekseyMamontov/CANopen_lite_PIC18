...
#define ERORR_OD_read error = 0x80; break;
#define OK_OD_read8   erorr = 0x4F;break;
#define OK_OD_read16 erorr = 0x4B;break;
#define OK_OD_read32 erorr = 0x43;break;
...
switch (tab->type){
	...
	if(sub_index ==  0 || (arr->sub_index) < sub_index ){ERORR_OD_read}else{sub_index--;}
	
	if(tab->object == UNSIGNED8)  {dump +=  *((uint8_t*)(arr->value)+(sub_index));OK_OD_read8} 
	if(tab->object == UNSIGNED16){dump += *((uint16_t *)(arr->value)+(sub_index));OK_OD_read16}
	if(tab->object == UNSIGNED32){dump =*((uint32_t *)(arr->value)+(sub_index));OK_OD_read32;}
	break;
	
	case OD_DEFSTRUCT:
		
		if(sub_index == 0 ){ERORR_OD_read}
	
		switch(tab->object){
		
		case PDO_COMM:
		   pdo_com = (struct PDO_comm *) tab->data;
		   if (pdo_com->sub_index < sub_index) {ERORR_OD_read};
		   if (sub_index == 1){dump   = pdo_com->cob_id;OK_OD_read32}	
		   if (sub_index == 2){dump += pdo_com->Transmission_type;OK_OD_read8};
		   if (sub_index == 3){dump += pdo_com->Inhibit_time;OK_OD_read16};	
		   if (sub_index == 5){dump += pdo_com->event_timer;OK_OD_read16};
		 break;			
	...
	if(tab->object == PDO_MAPPING){}
	if(tab->object == SDO_PARAMETER){}	
	if(tab->object == IDENTITY){}	
...
msg->frame.data4 =  d&0xff;
msg->frame.data5 = (d&0xff00) >>8;
msg->frame.data6 = (d&0xff0000)>>16;
msg->frame.data7 = (d&0xff000000)>>24;
};
